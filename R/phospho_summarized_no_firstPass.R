#'
#' @description Combines `set_metadata()`, `get_phospho_log()` and `get_stats()` into a single function. This is used if there is no stats report file created for the First-Pass search results for a DIA-NN search with phosphorylation set as a variable mod of STY.
#'
#' @param log_data The log file. Needs to be read into R using `readr::read_tsv` prior.
#' @param stats_tsv The stats report file. Needs to be read into R using "readr::read_tsv()" prior. This should just be the MBR report.
...
#' @importFrom magrittr %>%
#' 
phospho_summarized_no_firstPass <- function(log_data,
                                            stats_tsv,
